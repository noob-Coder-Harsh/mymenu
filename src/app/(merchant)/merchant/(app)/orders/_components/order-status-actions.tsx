    ...
    <div className="flex flex-col gap-2">
      {primary ? (
        <button
          ...
          className="flex h-12 items-center justify-center rounded-2xl bg-accent text-base font-semibold text-accent-foreground disabled:opacity-60"
        >
      ...
      {showPaidDoneSecondary ? (
        <button
          ...
          className="flex h-11 items-center justify-center rounded-2xl border border-border bg-surface text-sm font-medium disabled:opacity-60"
        >
          {busy ? "Updating…" : "Paid & done"}
        </button>
      ) : null}
      {canCancel ? (
        <button
          ...
          className="flex h-11 items-center justify-center rounded-2xl border border-border bg-surface text-sm font-medium text-danger disabled:opacity-60"
        >
          Cancel order
        </button>
      ) : null}
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col gap-1">
      <button
        ...
        disabled={busy}
        onClick={() => {
          if (busy || !window.confirm("Cancel this order?")) {
            return;
          }
          setError(null);
        ...
        className={
          className ??
          "flex h-11 w-full items-center justify-center rounded-2xl border border-border bg-surface text-sm font-medium text-danger disabled:opacity-60"
        }
      >
